...
public:
    int lengthOfLIS(vector<int>& nums) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            for (int j = 0; j < i; j++) {
                if (nums[i] > nums[j]) {
                    dp[i] = max(dp[i], dp[j] + 1);/**<extend up -300>![](https://labuladong.online/algo/images/lis/7.jpeg) */
                }
            }
        }
...
var lengthOfLIS = function(nums) {
    ...
    for (let i = 0; i < nums.length; i++) {
        for (let j = 0; j < i; j++) {
            if (nums[i] > nums[j]) {
                dp[i] = Math.max(dp[i], dp[j] + 1);/**<extend up -300>![](https://labuladong.online/algo/images/lis/7.jpeg) */
            }
        }
    }
